...
// Demonstrates how to load map showing only one placemark
...
public class SingleMarkerIDFragment extends Fragment implements MeridianRequest.Listener<Placemark>, MeridianRequest.ErrorListener {
    ...
    public void onResponse(Placemark placemark) {
        ...
        if (getView() != null) {
            getParentFragmentManager().beginTransaction().replace(((ViewGroup) getView().getParent()).getId(), SingleMarkerInteriorFragment.newInstance(placemark)).commitAllowingStateLoss();
        }
    }
