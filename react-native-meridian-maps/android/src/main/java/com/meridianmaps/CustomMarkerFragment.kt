...
class CustomMarkerFragment : MapFragment() {
    ...
    override fun markerForSelectedMarker(markerToSelect: Marker): Marker? {
        return if (markerToSelect is RandMarker && context != null) RandSelectedMarker(requireContext(), markerToSelect) else super.markerForSelectedMarker(markerToSelect)
    }
    ...
    private class RandMarker(context: Context, mapInfo: MapInfo, random: Random) : Marker(random.nextInt(mapInfo.width.toInt()).toFloat(), random.nextInt(mapInfo.height.toInt()).toFloat()) {
         ...
         * @return drop shadow
         */
        override fun getDropShadowBitmap(): Bitmap {
            return BitmapFactory.decodeResource(appContext.resources, R.drawable.selected_beacon)
        }
    }
